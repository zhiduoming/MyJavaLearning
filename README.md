...
## 2026.4.10（没推进）
今天在搞学校数据结构课设，让我们基于agent来搞一个个性化旅游管理系统，  
又配了一天的环境，但是把数据库的模型搭建起来了，我主要负责数据库，然后还整理了课设PPT所需材料

## 2026.4.11打卡Day22
1. JavaWeb
* jdbc概念
* Mapper层，Controller层，Dao层
* 分层解耦
* Mybatis基本概念
今天继续搞课设，明天要汇报了，和组员讨论了一晚上，但最终好像也没商讨出什么。。。

## 2026.4.12（没推进）
今天汇报PPT和看别人PPT演示，学习人家的设计报告，直接汇报到晚上了。真的有大二学生刚刚学完C语言和数据结构就能够写出具有  
各种社交功能，各种个性化推荐功能，各种复杂算法（听都没听过），各种奇思妙想的优化，然后还能前后端结合  
（全栈开发）的系统吗，虽说是利用agent，但是我感觉我才学习到springboot，他们口中各种功能以前我还没  
啥概念，现在真正接触才意识到有多难搞，不知道他们的设计报告是单单建立于想法还是也能实际落地呢，咳咳，  
要是光会想的话，我感觉我好像也行~

工具：在电脑上配置好了Codex，尝试了一下vibe coding，掏腰包买了个plus，这东西不用不知道，用了之后还是太香了  
感觉AI真的能取代人类了，还帮我吧电脑上的隐疾全部修好了，以前有问题的软件也都修复了，太强了！！
但是真的只借助于智能体来完成我的课设（老师是这么要求的，可能就是专门锻炼我们的vibe coding能力）, 在智能体的选择  
使用，协调，分工，归并，提示词方面还是有很大的难度，主要是需求要写清楚，提示词工程也很重要，但我经常使用AI工具，  
感觉也不是太困难，过几天试一试和这个Codex配合一下。
...
* 21.合并连个有序链表
  ...
  * 解法：维护一个 dummy 节点和 pre 节点，pre 节点将数值小的节点存到结果集，然后后移，待一个链表走完之后，处理另一个链表剩余部分
  * 算法复杂度：时间：O（n+m）n 和 m 为两个链表的长度  空间：O（1）
  * 标签：B
...
* 使用 Docker 完成 Redis 和 MySQL 的部署
...
* 优化iuvs 项目前端页面
* 增加头像上传，评论区显示头像，优化用户中心展示
* 增加 Redis 缓存验证码
* 增加忘记密码功能
* 学习 Docker compose
* 使用 Docker 将项目部署在腾讯云服务器上
